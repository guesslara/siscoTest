...
class grid_numSeries_xml
{
   ...
   function inicializa_vars()
   {
      ...
      $this->arquivo     .= ".xml";
      $this->tit_doc      = "grid_numSeries.xml";
   ...
   function grava_arquivo()
   {
      ...
      if (isset($_SESSION['sc_session'][$this->Ini->sc_page]['grid_numSeries']['xml_name']))
      {
          $this->arquivo = $_SESSION['sc_session'][$this->Ini->sc_page]['grid_numSeries']['xml_name'];
          $this->tit_doc = $_SESSION['sc_session'][$this->Ini->sc_page]['grid_numSeries']['xml_name'];
          unset($_SESSION['sc_session'][$this->Ini->sc_page]['grid_numSeries']['xml_name']);
      }
      if (!$this->Grava_view)
      {
      ...
      while (!$rs->EOF)
      {
         $this->xml_registro = "<grid_numSeries";
         $this->id = $rs->fields[0] ;  
   ...
   function NM_export_serie()
   {
         ...
         {
             $this->serie = mb_convert_encoding($this->serie, "UTF-8");
         }
         $this->xml_registro .= " serie =\"" . $this->trata_dados($this->serie) . "\"";
   }
   //----- nombrecliente
   function NM_export_nombrecliente()
   {
         if ($_SESSION['scriptcase']['charset'] == "UTF-8" && !NM_is_utf8($this->nombrecliente))
         {
             $this->nombrecliente = mb_convert_encoding($this->nombrecliente, "UTF-8");
         }
         $this->xml_registro .= " nombrecliente =\"" . $this->trata_dados($this->nombrecliente) . "\"";
   }
   //----- almacenasociado
   function NM_export_almacenasociado()
   {
         if ($_SESSION['scriptcase']['charset'] == "UTF-8" && !NM_is_utf8($this->almacenasociado))
         {
             $this->almacenasociado = mb_convert_encoding($this->almacenasociado, "UTF-8");
         }
         $this->xml_registro .= " almacenasociado =\"" . $this->trata_dados($this->almacenasociado) . "\"";
   }
   //----- status
   ...
   function monta_html()
   {
      ...
      unset($_SESSION['sc_session'][$this->Ini->sc_page]['grid_numSeries']['xml_file']);
      if (is_file($this->Ini->root . $this->Ini->path_imag_temp . "/" . $this->arquivo))
      {
          $_SESSION['sc_session'][$this->Ini->sc_page]['grid_numSeries']['xml_file'] = $this->Ini->root . $this->Ini->path_imag_temp . "/" . $this->arquivo;
      }
?>
...
<HEAD>
 <TITLE>Series Capturadas :: XML</TITLE>
