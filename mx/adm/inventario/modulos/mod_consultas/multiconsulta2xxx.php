...
<?
		...
		for($i=0;$i<count($m_data);$i++){
			//echo $m_data[$i]."<br>";
			$sql="select id_radio,modelo,imei,serial,sim,lote,status,status_nextel,statusProceso,statusDesensamble,statusDiagnostico,statusAlmacen,statusIngenieria,statusEmpaque,lineaEnsamble 
...
?>
						<script language="javascript">
						agregar_filaX('<?=$reg["id_radio"]?>','<?=$reg["modelo"]?>','<?=$reg["imei"]?>','<?=$reg["serial"]?>','<?=$reg["sim"]?>','<?=$reg["lote"]?>','<?=$reg["status"]?>','<?=$noEnviar?>','<?=$reg["statusProceso"]?>','<?=$reg["statusDesensamble"]?>','<?=$reg["statusDiagnostico"]?>','<?=$reg["statusAlmacen"]?>','<?=$reg["statusIngenieria"]?>','<?=$reg["statusEmpaque"]?>','<?=$reg["lineaEnsamble"]?>');
						</script>
<?php
					}
				}else{
					echo "<br> 2.".$m_data[$i]." NO EXISTE";
				}
			} else{ echo "<br> 1. ".$m_data[$i]." NO EXISTE";	}
...
function agregar_filaX(id_lote2,nds,act,lot,mod,iot,st0,st1,ien,ica,status1,status2,status3,status4,linea){
