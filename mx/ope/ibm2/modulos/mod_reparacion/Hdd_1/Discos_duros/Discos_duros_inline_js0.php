<form name="F2" method=post 
               action="Discos_duros_inline.php" 
               target="_self"> 
<input type=hidden name="id_" value="<?php echo NM_encode_input($this->nmgp_dados_form['id_']); ?>">
...
<form name="F5" method="post" 
  ...
  <input type="hidden" name="nmgp_parms" value="<?php echo NM_encode_input($_SESSION['sc_session'][$this->Ini->sc_page]['Discos_duros_inline']['parms']); ?>"/>
  <input type="hidden" name="script_case_init" value="<?php echo NM_encode_input($this->Ini->sc_page); ?>"/> 
  <input type="hidden" name="script_case_session" value="<?php echo NM_encode_input(session_id()); ?>"> 
</form> 
...
function scInlineFormSend()
{
  ...
  {
    parent.scInlineFormReceive(oResp, <?php echo $_SESSION['sc_session'][$this->Ini->sc_page]['Discos_duros_inline']['inline_form_seq'] ?>);
    return true;
  }
  return false;
}
...
function NM_critica (form, campo, label, mask, tipo, intr, dec, min, max)
{
 ...
 NM_tab_crit = new NM_critica(6);
 NM_tab_crit[1] = new NM_critica('F1', 'no_parte_', 'No Parte', 'naomask', 'lista', 'cxab', 20, 'TUDO');
 NM_tab_crit[2] = new NM_critica('F1', 'capacidad_', 'Capacidad', 'naomask', 'lista', 'cxab', 20, 'TUDO');
 NM_tab_crit[3] = new NM_critica('F1', 'rpm_', 'Rpm', 'naomask', 'lista', 'cxab', 50, 'TUDO');
 NM_tab_crit[4] = new NM_critica('F1', 'interface_', 'Interface', 'naomask', 'lista', 'cxab', 50, 'TUDO');
 NM_tab_crit[5] = new NM_critica('F1', 'pulgada_', 'Pulgada', 'naomask', 'lista', 'cxab', 20, 'TUDO');
 NM_tab_crit[6] = new NM_critica('NM_final');
 NM_tab_crit_1 = new NM_critica(6);
 NM_tab_crit_1[1] = new NM_critica('F1', 'no_parte_', 'No Parte', 'naomask', 'lista', 'cxab', 20, 'TUDO');
 NM_tab_crit_1[2] = new NM_critica('F1', 'capacidad_', 'Capacidad', 'naomask', 'lista', 'cxab', 20, 'TUDO');
 NM_tab_crit_1[3] = new NM_critica('F1', 'rpm_', 'Rpm', 'naomask', 'lista', 'cxab', 50, 'TUDO');
 NM_tab_crit_1[4] = new NM_critica('F1', 'interface_', 'Interface', 'naomask', 'lista', 'cxab', 50, 'TUDO');
 NM_tab_crit_1[5] = new NM_critica('F1', 'pulgada_', 'Pulgada', 'naomask', 'lista', 'cxab', 20, 'TUDO');
 NM_tab_crit_1[6] = new NM_critica('NM_final');
