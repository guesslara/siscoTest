...
<title>Catalogo de Pruebas Funcionales</title>
...
<script language="javascript">
	...
	function start(){
		...
		$.ajax({
			...
			url:"cat_pruebas_funcionales2.php",
			data:"action=listar",
			beforeSend:function(){ 
	...
	function coloca_productos2(idd){
		...
		if (confirm("¿Desea guardar los cambio realizados en la Prueba Funcional "+idd+"?")){
			$.ajax({
				async:true,
				type: "POST",
				dataType: "html",
	...
	function guardar(){
		...
		if (d==""||d=="undefined"||d==null||p==""||p=="undefined"||p==null){ 
			$("#mensajeX").html("Error: Por favor no omita los campos: Clave, Diagnostico y Aplica a los Productos.");
			//alert("Error: Por favor no omita los campos: Clave, Diagnostico y Aplica a los Productos.");
			return;
		}
		var url="action=guardar&d="+d+"&p="+p+"&o="+o;	
		
		//alert(url);
		if(confirm("¿Desea guardar la Prueba Funcional?")){
			$.ajax({
				async:true,
				type: "POST",
				dataType: "html",
...
<div id="div_nuevo" style="width: 50%;">
	<div class="div_nuevoA" id="div_nuevoA" style="width: 60%;">&nbsp;Nueva Prueba Funcional.</div>
