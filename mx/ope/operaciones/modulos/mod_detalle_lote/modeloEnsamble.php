...
?>
		...
		public function listaMov($id_proyecto,$idUsuario){
			...
			if($exeConMov==false){
				...
				if($cuantosMov>0){
					?><br><table align="center" style="width:98%;font-size:10px; border:1px solid #000;">
					<tr>
						<th colspan=2>MOVIMIENTOS REALIZADOS</th>
						<td rowspan='<?=$cuantosMov;?>' style="text-align:center;"><input type="button" name="next" id="next" value="SIGUIENTE" onclick="formLotes('<?=$id_proyecto?>','<?=$idUsuario;?>');"/><input type="button" name="back" id="back" onclick="cerrarVentana('divMensajeCaptura','transparenciaGeneral1');" value="CANCELAR"/></td>
					</tr><?
					$colorL="#FFF";
					while($rowM=mysql_fetch_array($exeConMov)){
						...
						?><tr style="background:<?=$colorL;?>">
							<td style="text-align:center; width:20%"><input type="radio" name="movimientos" id="movimientos" value="<?=$rowM['id_mov'];?>"/></td>
							<td style="text-align:justify; width:38%"><?=$rowM["referencia"]?></td>
						</tr><?	
					}
					?>
						<tr>
