...
$pagAct=$_POST['pagAct'];
$pag=15;//pag es el limite de registros
$lim=$pag+$intervalo;
if($intervalo==0){
	?><script>
		$("#ant").hide();
...
if($paginasT==0 && $intervalo==0){
	...
	if($noReg>$pag){
		...
		$CON="SELECT * FROM detalle_lotes WHERE id_lote='".$idLote."' and (status='Empaque' and id_irre_wk!='N/A' ) or status='SCRAP' limit 0,15";
		$exeCon=mysql_query($CON,conectarBd());
	}else{
		?><script>
			$("#PAct").html("1");
	...
	$CON="SELECT * FROM detalle_lotes WHERE id_lote='".$idLote."' and (status='Empaque' and id_irre_wk!='N/A' ) or status='SCRAP' limit ".$intervalo.",15";
	$exeCon=mysql_query($CON,conectarBd());
	$noReg=mysql_num_rows($exeCon);
	?>
	<input type="hidden" id="pagAct" name="pagAct" value="<?=$pagAct;?>"/>
...
?>
		<div id="tablaIz" style="width:100%;float:left; height:100%;background:#fff;">
			<table class="tab">
				<tr class="azul">
