...
function insertaRe(){
	...
	for(var i=0;i<form.elements.length;i++){
		if(form.elements[i].id=="lanzadorB1"||form.elements[i].id=="lanzadorB2"||form.elements[i].id=="lanzadorB4"||form.elements[i].id=="dias_trans"||form.elements[i].id=="dias_ab"||form.elements[i].value=="Guardar Registro"){
	...
	for (var x=0;x<form.elements.length;x++){
		if(form.elements[x].id=="num_reclamo"||form.elements[x].id=="info_falla"||form.elements[x].id=="producto"||form.elements[x].id=="fecha_compra"||form.elements[x].id=="fecha_asignacion"||form.elements[x].id=="status"||form.elements[x].id=="num_contrato"||form.elements[x].id=="nombre_cliente"||form.elements[x].id=="numero_serie"){
...
function actualizaRe(id){
	...
	$("#dias_ab").attr("value",diferencia);
	
	}
	function sumarD(){
		
		fechasum=document.getElementById("fecha_asignacion").value.split("-");
		fechasum2=new Date(fechasum[0],fechasum[1]-1,fechasum[2])
		fechasum2.setDate(fechasum2.getDate()+30);
		
		
		var mes=fechasum2.getMonth()+1;
		var anio=fechasum2.getFullYear();
		var dia=fechasum2.getDate();
		
		
		//alert(typeof mes); para saber que tipo de variable es....
		mes=mes.toString();
		if(mes.length<2){
			mes="0".concat(mes);
		}
	...
	$("#fecha_compromiso").attr("value",anio+"-"+mes+"-"+dia);
	}
	function movimientoSerie(evento){
		
		if(evento.which==13){
			document.getElementById("info_falla").focus();
