...
<script type="text/javascript">
	$(document).ready(function(){
		diagnostica('<?=$_SESSION[$txtApp['session']['idProyectoSeleccionado']];?>','<?=$_SESSION[$txtApp['session']['idUsuario']];?>','Diagnosticar');
		//mostrarLotes('<?=$_SESSION[$txtApp['session']['idProyectoSeleccionado']];?>','Inicio');
		redimensionar();		
	});	
...
<div id="contenedorEnsamble">
	<div id="contenedorEnsamble3">
		<div id="barraOpcionesEnsamble">
			<div class="opcionesEnsamble" onclick="clean2();diagnostica('<?=$_SESSION[$txtApp['session']['idProyectoSeleccionado']];?>','<?=$_SESSION[$txtApp['session']['idUsuario']];?>','Diagnosticar')" title="Llenar reporte">Inicio</div>
			<div class="opcionesEnsamble" onclick="clean2();muestraSel('<?=$_SESSION[$txtApp['session']['idProyectoSeleccionado']];?>','<?=$_SESSION[$txtApp['session']['idUsuario']];?>')" title="General">General</div>
		</div>
			<div id="infoEnsamble3">
				<div id="listadoEmpaque2" style="border:1px solid #e1e1e1;background:#fff; height:99%;width:97%;font-size:12px;margin:3px;overflow: auto; display: none">
					<div id="selectListado" style="text-align: center;border:0px solid #e1e1e1; background: #fff; height: 30px; width:97%; font-size: 12px; margin:3px; overflow: :auto; display: none;">	
					</div>
					<div id="consultaListado" style="border:0px solid #e1e1e1; background: #fff; height:80%; width:97%; font-size: 12px; margin:3px; overflow: :auto; display: none;">
					</div>	
				</div>
...
<div id="transparenciaGeneral2" class="transparenciaGeneral" style="display:none;">
	<div id="ventanaDialogo1" class="ventanaDialogo" style="display:none;">
		...
		<div id="msgVentanaDialogo" class="msgVentanaDialogo" style="border: 0px solid #ff0000;height: 85%;overflow: auto;"></div>
		<div id="barraBotonesVentanaDialogo" align="center" style="display: none;">
			<input type="button" name="agrega" id="agrega" value="Agrega" onclick="recupera();cerrarVentana('ventanaDialogo1')"><input type="button" name="cerrar" id="cerrar" value="Cerrar" onclick="cerrarVentana('ventanaDialogo1')">
		</div>
	</div>
</div>
<?
