		...
		<div id="hoja" style="width:<?=$orientacion;?>">
			...
			<div id="contenidoHoja" >
				<?
							...
							$cSEN="SELECT count( id_Senc ) AS sencs FROM detalle_lotes where id_lote='".$idLote."' and status='Empaque' and id_irre_wk='N/A' and TipoEntrada like '%Garant%' ORDER BY (id_Senc)";
							$exeCS=mysql_query($cSEN,conectarBd());
							$rowCS=mysql_fetch_array($exeCS);
							$comm="SELECT count( detalle_lotes.id_commodity ) AS qty, detalle_lotes.id_commodity, CAT_commodity.desc_esp FROM detalle_lotes INNER JOIN CAT_commodity ON detalle_lotes.id_commodity = CAT_commodity.id_commodity WHERE detalle_lotes.id_lote='".$idLote."' AND status='Empaque' and id_irre_wk='N/A' and TipoEntrada like '%Garant%'GROUP BY (detalle_lotes.id_commodity)";
							$exeComm=mysql_query($comm,conectarBd());
							$poPar="SELECT count(CAT_SENC.NoParte) as Total, CAT_SENC.NoParte FROM detalle_lotes INNER JOIN CAT_SENC ON CAT_SENC.id_SENC= detalle_lotes.id_Senc WHERE detalle_lotes.id_lote='".$idLote."' and status='Empaque' and id_irre_wk='N/A' and TipoEntrada like '%Garant%' GROUP BY(NoParte)";
